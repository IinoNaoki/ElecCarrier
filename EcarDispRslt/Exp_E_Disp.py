...
y_a1_avg_bell = [RESset_bell[i][1] for i in range(expnum)]
y_a2_avg_bell = [RESset_bell[i][2] for i in range(expnum)]
y_e_steady_bell = [RESset_bell[i][3] for i in range(expnum)]

y_v_avg_myo = [RESset_myo[i][0] for i in range(expnum)]
y_a1_avg_myo = [RESset_myo[i][1] for i in range(expnum)]
y_a2_avg_myo = [RESset_myo[i][2] for i in range(expnum)]
y_e_steady_myo = [RESset_myo[i][1] for i in range(expnum)]

y_v_avg_side = [RESset_side[i][0] for i in range(expnum)]
y_a1_avg_side = [RESset_side[i][1] for i in range(expnum)]
y_a2_avg_side = [RESset_side[i][2] for i in range(expnum)]
...
plot(x_axis_list,y_a1_avg_bell,color='red',marker='o',label='MDP')
plot(x_axis_list,y_a1_avg_myo,color='green',marker='^',label='MYO')
plot(x_axis_list,y_a1_avg_side,color='black',marker='s',label='SIDE')
plot(x_axis_list,y_a2_avg_bell,color='red',linestyle='--',marker='o',label='MDP')
plot(x_axis_list,y_a2_avg_myo,color='green',linestyle='--',marker='^',label='MYO')
plot(x_axis_list,y_a2_avg_side,color='black',linestyle='--',marker='s',label='SIDE')
